export async function initDefektas({ supabase, user, profile }) {
  ...
  function getPhotoBlockForCategory(category) {
    const input = getRequiredPhotoInput(category);
    return input?.closest('.bg-slate-900\\/50') || input?.parentElement || null;
  }

  function updatePhotoStatus(category) {
    const normalized = normalizeCategory(category);
    const displayCategory = normalized === 'document' ? 'doc' : normalized;
    const status = form.querySelector(`[data-photo-status="${displayCategory}"]`);
  ...
  function validateRequiredFields(formData) {
    ...
    const missingPhotos = requiredPhotos.filter(item => {
      const category = item.input?.dataset?.category || item.label;
